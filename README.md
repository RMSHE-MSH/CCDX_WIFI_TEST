...
- **无效密码跟踪**：程序在测试中发现某个特定用户名所尝试的密码无效时，会自动保存这些信息到`InvalidAccounts.csv`文件中。这样做旨在提高测试效率并节约请求资源。当程序在下次测试同一用户名时，将自动跳过其所有已知的无效密码，避免重复测试。
...
| 2024.03.22 | Beta.2024.03.22.Mark0 | **1.** 更新`无效密码跟踪`功能, 程序会自动记录测试中某特定用户名尝试过的无效密码，以确保在后续对同一用户名的测试中跳过这些已知的无效密码，从而避免重复测试并提高测试效率。 |

...
| 2024.03.21 | Beta.2024.03.21.Mark0 | **1.** 调整测试参数默认值, 提高测试效率.<br/>**2.** 更新`有效账号跟踪`功能, 程序在生成用户名列表阶段自动排除已验证的有效账户, 避免重复测试. |
| 2024.03.22 | Beta.2024.03.22.Mark0 | **1.** 更新`无效密码跟踪`功能, 程序会自动记录测试中某特定用户名尝试过的无效密码，以确保在后续对同一用户名的测试中排除这些已知的无效密码，从而避免重复测试并提高测试效率。 |
| 2024.03.23 | Beta.2024.03.22.Mark1 | **1.** 优化测试流程, 通过在密码测试前完成所有`无效密码跟踪`工作, 提高测试效率.<br/>**2.** 修复若干Bug, 及时释放无用内存, 改善控制台信息显示. |
...
int main() {
    ...
    // 示例设定：使用随机增量模式("ir")，密码尝试次数为100次, 生成用户名数量为500, 最大递减值设为10，;
    WIFI_Test test("ir", 100, 500, 8143086109, 9390026092, 10);
    // 执行runTests方法以进行测试
    test.runTests();
    return 0;
}
